/**
 * Architecture templates — the 4 reference architectures the AI uses
 * as starting points. Enhanced with service selection decision tree insights.
 */

export const TEMPLATES = `## Architecture Templates
...
Best for: Traditional web applications, CRUD apps, portals, dashboards, content management systems.
Compute choice: app-service (default). Switch to container-apps if custom Docker images or sidecar patterns needed.
...
- app-service (API Backend): "Premium v3 P1 App Service for RESTful API layer. Handles auth validation, rate limiting, and business logic. Zone-redundant with auto-scale 2-6 instances. Accesses data via managed identity. ~$250/mo."
- azure-sql (SQL Database): "Business Critical S3 with zone-redundant HA, automatic failover group, TDE encryption, and 35-day point-in-time restore. Accessed via managed identity + private endpoint. ~$450/mo."
...
- key-vault (Key Vault): "Standard tier Key Vault with soft-delete and purge protection. Stores connection strings, API keys, TLS certs, and encryption keys. All access via managed identity. ~$5/mo."
...
- entra-id (Entra ID): "P1 Entra ID with OAuth 2.0/OIDC, conditional access policies, MFA enforcement, and RBAC roles for admin/user separation. Workload identity for service-to-service auth. Included in M365."
...
- function-app (Background Worker): "Consumption plan Function App triggered by Service Bus. Handles email dispatch, PDF generation, and scheduled cleanup tasks. Uses managed identity for Key Vault and data access. ~$5/mo."
...
Best for: Distributed systems, independent team ownership, polyglot services, event-driven architectures.
Compute choice: container-apps (default). Switch to aks if >20 microservices, need service mesh, or require node-level control.
...
- container-apps (Orders Service): "Container Apps with min 2 / max 10 replicas. Processes order creation, payment orchestration, and fulfillment workflows. Managed identity for all downstream access. ~$75/mo."
...
- cosmos-db (Cosmos DB): "Multi-region serverless Cosmos DB (NoSQL API) for orders, profiles, and catalogs. Automatic failover, 99.999% SLA with multi-region writes. Accessed via managed identity. ~$200/mo."
- azure-sql (SQL Database): "Business Critical tier for inventory and transaction reporting with zone-redundant HA, read replicas, and TDE encryption. Accessed via managed identity + private endpoint. ~$450/mo."
...
- entra-id (Entra ID): "P1 Entra ID with OAuth 2.0 for customers, admin RBAC, workload identity for service-to-service auth, and conditional access policies for admin portal."
...
Best for: IoT platforms, real-time analytics, event sourcing, ETL/ELT pipelines, streaming architectures.
Compute choice: function-app (Premium EP1 for stream processing, Consumption for batch ETL).
...
- function-app (Stream Processor): "Premium EP1 with zone redundancy. Real-time stream processing — enrichment, filtering, windowed aggregations. KEDA-scaled by Event Hub lag. Uses managed identity. ~$150/mo."
...
- cosmos-db (Event Store): "Serverless Cosmos DB with change feed enabled. Append-only event store for event sourcing and CQRS read projections. 99.99% SLA. Accessed via managed identity. ~$100/mo."
...
- entra-id (Entra ID): "Workload identity for inter-service authentication and RBAC for pipeline management. Conditional access for admin operations."
...
Best for: Conversational AI, document Q&A, knowledge management, AI-powered search, copilot-style apps.
Compute choice: app-service (for web UI + orchestration API). function-app for document indexing pipeline.
...
- app-service (API Layer): "Premium v3 P2 orchestration API managing RAG pipeline — retrieval, prompt construction, and LLM calls. Auto-scale 2-8 instances for token-heavy workloads. Uses managed identity for all downstream services. ~$500/mo."
...
- cosmos-db (Cosmos DB): "Serverless Cosmos DB (NoSQL API) for conversation history, user preferences, and metadata. Change feed for real-time analytics. 99.99% SLA. Accessed via managed identity. ~$100/mo."
...
- function-app (Indexer): "Premium EP1 triggered on blob upload. Chunks documents, generates embeddings via OpenAI, and indexes into AI Search. Uses managed identity. ~$150/mo."
...
- entra-id (Entra ID): "P1 Entra ID with OAuth 2.0, RBAC (admin/user/reviewer roles), conditional access, and workload identity for all service-to-service auth."
